...
public class SoftLayerDualXeon5500SeriesComputeServiceAdapterLiveTest  extends BaseSoftLayerClientLiveTest {


	   private SoftLayerComputeServiceAdapter adapter;
	   private TemplateBuilder templateBuilder;
	   private Factory sshFactory;
	   private NodeAndInitialCredentials<SoftLayerNode> server;
	   ...
	   public void testListNodes() {
		   Iterable<SoftLayerNode> listNodes = adapter.listNodes();
		   for (SoftLayerNode softLayerNode : listNodes) {
			   adapter.getNode(softLayerNode.getId() + "");
		   }
	   }
	   ...
	   public void testCreateNodeWithGroupEncodedIntoNameThenStoreCredentials() {
	      ...
	      templateBuilder.locationId("37473").hardwareId("2091,1956,1267,272,36");
	      Template template = templateBuilder.build();
	      // test passing custom options
	      template.getOptions().as(SoftLayerTemplateOptions.class).domainName("adaml.org");
	   ...
	   protected void doConnectViaSsh(SoftLayerNode server, LoginCredentials creds) {
	      SshClient ssh = sshFactory.create(HostAndPort.fromParts(server.getPrimaryIpAddress(), 22), creds);
	      try {
	         ssh.connect();
	   ...
	   protected Properties setupProperties() {
	      ...
	      properties.putAll(new DualXeon5500SeriesProperties().sharedProperties());
